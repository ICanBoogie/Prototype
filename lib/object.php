...
class PropertyNotDefined extends PropertyError
{
	public function __construct($message, $code=500, \Exception $previous=null)
	{
		if (is_array($message))
		{
			...
			{
				$message = sprintf('Unknown property "%s" for object of class "%s".', $property, get_class($container));
			}
			else if (is_array($container))
			{
				$message = sprintf('Unknown index "%s" for the array: %s', $property, $container);
			}
			else
			{
				$message = spritnf('Unknown property "%s".', $property);
			}
		}
...
class PropertyNotReadable extends PropertyError
{
	public function __construct($message, $code=500, \Exception $previous=null)
	{
		if (is_array($message))
		{
			...
			if (is_object($container))
			{
				$message = sprintf('The property "%s" for object of class "%s" is not readable.', $property, get_class($container));
			}
			else if (is_array($container))
			{
				$message = sprintf('The index "%s" is not readable for the array: %s', $property, json_encode($container));
			}
			else
			{
				$message = sprintf('The property "%s" is not readable.', $property);
			}
		}
...
class PropertyNotWritable extends PropertyError
{
	public function __construct($message, $code=500, \Exception $previous=null)
	{
		if (is_array($message))
		{
			...
			if (is_object($container))
			{
				$message = sprintf('The property "%s" for object of class "%s" is not writable.', $property, get_class($container));
			}
			else if (is_array($container))
			{
				$message = sprintf('The index "%s" is not writable for the array: %s', $property, json_encode($container));
			}
			else
			{
				$message = sprintf('The property "%s" is not writable.', $property);
			}
		}
